...
from ..forms.manage_post_forms import ManagePostCreateForm, ManagePostUpdateForm
...
class AdminPostCreateView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
             ...
             services.post_service.admin_create_post(**post_data) 
...
#             'content_type': form.cleaned_data['content_type'],
#             # 'media_url': request.POST.get('media_url', ''), 
#             'title': form.cleaned_data['title'], # type: ignore
#             'description': form.cleaned_data['description'], # type: ignore
#             'accessability': form.cleaned_data['accessability'], # type: ignore
#             # 'members': request.POST.getlist('members'),  
#             'treat_as': form.cleaned_data['treat_as'], # type: ignore
#             # 'is_active': request.POST.get('is_active', 'on') == 'on'
#             return redirect('post_list')
#         return render(request, 'adminuser/post/update.html', {"form": form, "user_id": user.id})
...
class AdminPostUpdateView(View):
    ...
    def post(self, request, post_id):
        ...
        if form.is_valid():
            # post.posted_by = form.cleaned_data['posted_by']
